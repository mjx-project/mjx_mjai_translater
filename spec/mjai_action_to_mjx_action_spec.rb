...
RSpec.describe "mjai_action_to_mjx_action" do
    ...
    it "discard" do
        ...
        mjai_action = {:type=>:dahai, :actor=>players[0], :pai=>Mjai::Pai.new("1m"), :tsumogiri=>false}  # 今は起家から順番に恣意的に0,1,2,3とidを決めている。trans_server のインスタンス変数がその対応を全て管理しているので別の問題
    ...
    it "tsumogiri" do 
        ...
        legal_actions = observation.legal_actions
        mjai_action = {:type=>:dahai, :actor=>players[0], :pai=>Mjai::Pai.new("W"), :tsumogiri=>true} 
    ...
    it "chi" do
        ...
        legal_actions = observation.legal_actions
        mjai_action = {:type=>:chi, :actor=>players[0], :target=>players[3], :pai=>Mjai::Pai.new("3m"), :consumed=>[Mjai::Pai.new("2m"), Mjai::Pai.new("4m")]}
        expect(MjaiToMjx.new(absolutepos_id_hash).mjai_act_to_mjx_act(mjai_action, legal_actions)).to eq legal_actions[0]
    end
    it "pon" do
        observation = observation_from_json(lines,5)
        legal_actions = observation.legal_actions
        mjai_action = {:type=>:pon, :actor=>players[0], :target=>players[1], :pai=>Mjai::Pai.new("4p"), :consumed=>[Mjai::Pai.new("4p"), Mjai::Pai.new("4p")]}
        expect(MjaiToMjx.new(absolutepos_id_hash).mjai_act_to_mjx_act(mjai_action, legal_actions)).to eq legal_actions[0]
    end
    it "kakan" do
        observation = observation_from_json(lines,33)
        legal_actions = observation.legal_actions
        mjai_action = {:type=>:kakan,:actor=>players[0],:pai=>Mjai::Pai.new("N"),:consumed=>[Mjai::Pai.new("N"), Mjai::Pai.new("N"), Mjai::Pai.new("N")]}
        expect(MjaiToMjx.new(absolutepos_id_hash).mjai_act_to_mjx_act(mjai_action, legal_actions)).to eq legal_actions[0]
    end
    it "daiminkan" do
        observation = observation_from_json(lines,22)
        legal_actions = observation.legal_actions
        mjai_action = {:type=>:daiminkan, :actor=>players[0], :target=>players[2], :pai=>Mjai::Pai.new("2p"), :consumed=>[Mjai::Pai.new("2p"), Mjai::Pai.new("2p"), Mjai::Pai.new("2p")]}
        expect(MjaiToMjx.new(absolutepos_id_hash).mjai_act_to_mjx_act(mjai_action, legal_actions)).to eq legal_actions[1]
    end
    it "ankan" do
        observation = observation_from_json(lines_1,29)
        legal_actions = observation.legal_actions
        mjai_action = {:type=>:ankan,:actor=>players[1],:consumed=>[Mjai::Pai.new("P"), Mjai::Pai.new("P"), Mjai::Pai.new("P"), Mjai::Pai.new("P")]}
        expect(MjaiToMjx.new(absolutepos_id_hash).mjai_act_to_mjx_act(mjai_action, legal_actions)).to eq legal_actions[0]
    end
    it "riichi" do
        observation = observation_from_json(lines,129)
        legal_actions = observation.legal_actions
        mjai_action = {:type=>:reach,:actor=>players[0]}
        expect(MjaiToMjx.new(absolutepos_id_hash).mjai_act_to_mjx_act(mjai_action, legal_actions)).to eq legal_actions[0]
    end
    it "tsumo" do
        observation = observation_from_json(lines,153)
        legal_actions = observation.legal_actions
        mjai_action = {:type=>:hora,:actor=>players[0],:target=>players[0],:pai=>Mjai::Pai.new("8s")}
        expect(MjaiToMjx.new(absolutepos_id_hash).mjai_act_to_mjx_act(mjai_action, legal_actions)).to eq legal_actions[0]
    end
    it "ron" do
        observation = observation_from_json(lines,53)
        legal_actions = observation.legal_actions
        mjai_action = {:type=>:hora,:actor=>players[0],:target=>players[3],:pai=>Mjai::Pai.new("7m")}
        expect(MjaiToMjx.new(absolutepos_id_hash).mjai_act_to_mjx_act(mjai_action, legal_actions)).to eq legal_actions[0]
    end
    it "none" do
        observation = observation_from_json(lines,5)
